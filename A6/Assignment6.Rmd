...
melted_cc <- melt(cc)
t <- ggplot(data = melted_cc, aes(x=X1, y=X2, fill=value)) + geom_tile(color = "white") +  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
t
...
de <- decideTestsDGE(qlfPairedTreatControlvsProtac_loess, p=0.05, adjust='BH')
detags <- rownames(qlfPairedTreatControlvsProtac_loess)[as.logical(de)]
...
de <- decideTestsDGE(qlfPairedTime6vs24_loess, p=0.05, adjust='BH')
detags <- rownames(qlfPairedTime6vs24_loess)[as.logical(de)]
...
#ANSWER: The first normalization utilized scaling factors based on the weighted trimmed mean of M-values (TMM) to convert raw library sizes into effective library sizes. The differences between TMM and loess normlization are especially visible in the MA plots that compare the treatment status of samples. In MA plots, peaks that are significantly differential are displayed in red. 

#s it depicts more significant differentially accessible regions in control samples than Protac. Furthermore, the loess-based method depicts a slight increase in accessibility, rather than the decrease observed with TMM normalization. In my opinion, the loess method is more reliable in this case because . The TMM normalization method assumes that most regions of the genome are not truly differentially accessible, and should be applied when signal differences are presumed to be caused by technical artifacts or systematic biases in library ATAC distribution (e.g., mapping errors, PCR duplicates). On the other hand, loess-based normalization assumes a symmetric global distribution in which there are no true biological global differences in ATAC reaction efficiency or distribution, and any evidence of these biases are technical and should be removed. normalizes the signal distribution locally based on extent of ATAC signal abundance. As a result, the loess fit 
...
keep <- c("BI_protac","control","region")
filtered_atacSeqData_CvsP <- filtered_atacSeqData[,grepl(paste(keep, collapse = "|") , names(filtered_atacSeqData) )]

atac_GRanges <- GRanges(filtered_atacSeqData_CvsP$region)
...
filtered_atacSeqData_CvsP$GC <- letterFrequency(dna_sequences, "GC", as.prob=TRUE)
...
cpm_filtered_atacSeqData$region <- filtered_atacSeqData_CvsP$region
cpm_filtered_atacSeqData$GC <- filtered_atacSeqData_CvsP$GC
melted_cpm_atacSeqData <- melt(cpm_filtered_atacSeqData, id.vars=c("region","GC"))
u <- ggplot(melted_cpm_atacSeqData, aes(x=GC, y=value)) + geom_point() + ylim(0,50) + stat_smooth(method = "gam")
