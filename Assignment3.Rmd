...
sambamba view -F "[XS] == null and not unmapped and not duplicate" -q -
S -c bowtie2_output.sam
...
# ANSWER: Yes, the run type makes a difference. In fact, the number of uniquely mapped reads for the paired-end file is nearly twice as large as the number of uniquelt mapped reads for the single-end file. This is because each mate is aligned separately, and the information on both pairs is combined and reported in the same alignment line.Sambamba reports each mate in a pair separately. Arguably, a better comparison metric would surface from applying the filters "[XS] == null and not unmapped and not duplicate and first_of_pair", "[XS] == null and not unmapped and not duplicate and second_of_pair", or "[XS] == null and not unmapped and not duplicate and not mate_is_unmapped" to the paired-end file. This would amount to 1346202, 1340033, and 2672845 respectively. The latter can be divided by two (1336422.5) to compose Sambamba outputs which are more suitable to compare against the single-end value of 1325510.
